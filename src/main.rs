...
impl Config {
  pub fn from_env() -> Result<Self> {
    Ok(Config {
      ...
      supabase_key: std::env::var("SUPABASE_ANON_KEY").expect("SUPABASE_ANON_KEY must be set"),
...
fn schema() -> UpdateHandler<anyhow::Error> {
  ...
  let message_handler = Update::filter_message()
    ...
    .branch(dptree::filter(|msg: Message| msg.photo().is_some()).endpoint(slip::handle_slip_image))
    ...
    Some((item, amount)) => {
      ...
      .await
      {
        ...
        Err(e) => {
          ...
          bot
            .send_message(chat_id, "❌ บันทึกไม่สำเร็จ กรุณาลองใหม่")
            .await?;
        }
      }
    }
